...
class LevelData {
  ...
  //level test làm tiểu luận
  List<ObstacleData> levelTest1() {
    List<ObstacleData> level = [];
  ...
  List<ObstacleType?> randomRowObstacles() {
    ...
    for (int i = 0; i < result.length; i++) {
      if (result[i] == null) {
        ...
        //ở hàm này mông muốn là những cành cay giống như là mộc ra từ những cái cây 2
    ...
    //chắc chắn người chơi phải có ít nhất 1 ô trống để có thể vượt qua
    if (!result.contains(null)) {
      ...
      while (!hasReplaced) {
        ...
        //không thay thế vật cản không phải branch
        if (result[replaceIndex] != ObstacleType.branch) {
          result[replaceIndex] = null;
          hasReplaced = true; // Đã thay thế thành công
        }
      }
    }
